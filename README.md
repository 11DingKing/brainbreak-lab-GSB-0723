...
# 真实 PostgreSQL 集成测试（用一次性容器）
docker run -d --rm --name focuslab-pg -e POSTGRES_PASSWORD=focus -e POSTGRES_DB=focuslab -p 55432:5432 postgres:16-alpine
FOCUS_PG_DSN='postgres://postgres:focus@localhost:55432/focuslab?sslmode=disable' go test -race ./internal/pgstore/
docker stop focuslab-pg
```
...
- **时区跨日**：本地日历日分桶、跨午夜时长拆分、时区边界年龄（[fold_test.go](internal/domain/fold_test.go)）
- **单次额度截断**：单次限制真正削减 allowed 用量而非仅记录违规（[fold_test.go](internal/domain/fold_test.go)）
...
- **PG 集成**（真实数据库，[integration_test.go](internal/pgstore/integration_test.go)）：
  - 同一受试者跨设备并发写入**不同**事件 → 全部收敛、无漏计（回归 lost-update）
  - 迟到事件并发修正 → 最终结果等于全量重放
  - 跨午夜拆分、单次额度截断、事务回滚零残留

### 并发正确性说明

PostgreSQL 的 READ COMMITTED 下，两个事务写入**不同**事件（幂等键不同、行级无冲突）时，各自 `ListEvents` 都看不到对方未提交的行，随后都覆盖 `results` → 后写者胜出、漏计事件。修复用 `SELECT … FOR UPDATE` 对 subject 行加锁（[LockSubjectForUpdate](internal/pgstore/pgstore.go)），令同一受试者的写入/重算串行化，保证重算能观察到全部已提交事件。

事务内解密个人数据改用事务自身连接（[GetDataKey](internal/pgstore/pgstore.go)），避免持锁事务再申请第二个连接池连接，从而消除并发达到连接池大小时的死锁。
